...
class _ComplianceScreenState extends State<ComplianceScreen> {
  ...
  Future<void> _requestExport() async {
    ...
    if (session == null) {
      setState(() {
        _status = null;
        _error = 'Sign in before requesting data export.';
      });
      return;
    }
    setState(() {
      _submittingExport = true;
      _status = null;
      _error = null;
    });
    final api =
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _status = 'Data export request queued: ${response.requestId}';
      });
    } catch (error) {
      if (!mounted) return;
      ...
      api.close();
      if (mounted) {
        setState(() => _submittingExport = false);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        const Text(
          'You can request data export or account deletion. Requests are authenticated, audited, queued, and processed against user-owned profile, chat, memory, upload, session, and related private records.',
        ),
        const SizedBox(height: 12),
        FilledButton.icon(
          onPressed: _submittingExport ? null : _requestExport,
          icon: const Icon(Icons.download_outlined),
          label: Text(_submittingExport
              ? 'Requesting export...'
              : 'Request data export'),
        ),
        const SizedBox(height: 8),
