...
register_user() {
  ...
  curl -fsS -X POST "$api_base/auth/register" \
    ...
    -d "$(jq -n --arg email "$email" '{email:$email,password:"StrongPassword123!",display_name:"Feature Entitlement Proof User",provider:"password",provider_user_id:$email,metadata:{}}')"
}
